...
$customerProductInfo->customerId = $_GET['customerId'];

$result1 = $customerProductInfo->GetAllProductIdByCustomerId();
...
if($num>0){
	...
	$result = $customerProductInfo->GetAllProductIdByCustomerIdAndProductid();
	...
	if($result){
	  ...
	  if($num>0){
	    ...
	    array_push($arrangedArray,$customerProductResultArray);
	  }else{
	...
	echo json_encode($arrangedArray);
}else{
	$customerProductInfo->error = "No records found";
	echo json_encode($customerProductInfo);
}
