...
$Chat->roleId = $_GET['roleId'];

$result = $Chat->GetAllLastChatWithCustomerDetailsByAdminId();
...
if($result){
  ...
  if($num>0){
      ...
      {          		          
       ...
       $chatResultArray = array();
      
      foreach($ChatArray as $array){   
			...
			array_push($chatResultArray,$array);
						
	  }
    
      echo json_encode($chatResultArray); //converting the output data into JSON
  }else{
