...
if(isset($_GET['additionalParams'])){
	$additionalJson =  json_decode($_GET['additionalParams'],true);		
	//$additionalJson =  json_decode($_GET['additionalParams'],true);		
}else{
	$additionalJson = '';
}
...
if (strpos($notificationDetails->notificationPerson, 'L') !== false) {
	...
	$data = array("nativeTitle" => $notificationDetails->nativeTitle, "nativeMessage" => $notificationNativeMessage,"title" => $notificationDetails->title, "message" => $notificationMessage,"apiKey"=>$apiKey,"pushType"=>"individual","key"=>$customerDetails->tokenId,"language"=>$customerDetails->LanguageId,"deviceType"=>$customerDetails->DeviceType,"notificationType" => $notificationDetails->type,"additionalData" => $additionalJson);
	$options = array(
	'http' => array(
	'header'  => "Content-type: application/x-www-form-urlencoded\r\n",
	'method'  => 'POST',
	...
	$url = $apiRootPath.'SendPushNotification.php';
	$data = array("nativeTitle" => $notificationDetails->nativeTitle, "nativeMessage" => $notificationNativeMessage,"title" => $notificationDetails->title, "message" => $notificationMessage,"apiKey"=>$apiKey,"pushType"=>"individual","key"=>$vendorDetails->tokenId,"language"=>$vendorDetails->LanguageId,"deviceType"=>$vendorDetails->DeviceType,"notificationType" => $notificationDetails->type,"additionalData" => $additionalJson);
