...
if($data->isReopen==1){
	...
	if($availableVendor->error != "No records found"){
		$service->vendorId = 3; //$availableVendor->vendorId;	
	}else{
...
if ($availableLogistics->error != "No records found") {
	$service->logisticsId = 7; //$availableLogistics->id;
} else {
...
if($result){
	    ...
	    if($service->tempServiceId!=0){
			...
			$data = array("tempServiceId" => $service->tempServiceId);		
			$options = array(
			    'http' => array(
			        'header'  => "Content-type: application/x-www-form-urlencoded\r\n",
			        'method'  => 'POST',
		...
		foreach($data->itemDetails as $item){			
			...
			$data = array("serviceId" => $service->serviceId, "productId" => $service->productId, "itemId" => $item->itemId,"isActive" => 1, "type"=>$item->type,"name" => $item->name,"optionAnswer"=>$item->optionAnswer);		
			$options = array(
			    'http' => array(
			        'header'  => "Content-type: application/x-www-form-urlencoded\r\n",
			        'method'  => 'POST',
