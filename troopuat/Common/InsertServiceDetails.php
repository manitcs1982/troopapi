...
//$service->logisticsId = 1;
	...
	if ($result) {
		...
		//1st SR
		 ...
		 $url = $apiRootPath . 'GetAllServiceCountByCustomerId.php?customerId=' . $service->customerId;
		$options = array(
			'http' => array(
				'header' => "Content-type: application/x-www-form-urlencoded\r\n",
				'method' => 'GET',
		...
		$context = stream_context_create($options);
		$result = file_get_contents($url, false, $context);
		$SRCountArray = json_decode($result, true);
		 //json_encode($SRCountArray);



		if ($SRCountArray['SRCount'] == 1) {
			 $url = $apiRootPath . 'GetDiscountMasterByName.php?discountName=1stSR';
			$options = array(
				'http' => array(
					'header' => "Content-type: application/x-www-form-urlencoded\r\n",
					'method' => 'GET',
			...
			$data2 = array("serviceId" => $service->serviceId, "discountMasterId" => $t['discountMasterId'], "name" => $t['discountName'], "description" => $t['discountDescription'], "notes" => $t['discountReason'], "amount" => $t['discountAmount'], "isActive" => $t['discountIsActive']);
			$options = array(
				'http' => array(
					'header' => "Content-type: application/x-www-form-urlencoded\r\n",
					'method' => 'POST',
